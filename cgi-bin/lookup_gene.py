...
with con:
    ...
    cur.execute( """SELECT  *   FROM    HTTP,TFBS,GENE,GENE2TFBS 
                                ...
                                ORDER BY TFBS.organism,TFBS.disease,TFBS.experiment""" % (form_data.upper()))
    rows = cur.fetchall()
    
    for row in rows:
        row['TFBS_ID']="<div id=\"%s\"><a onclick='tfbsdata(\"%s\")' href='print_tfbs_data.py?id=%s'target=\"_blank\">tfbs%s</a></div>" % (row['TFBS_ID'],row['TFBS_ID'],row['TFBS_ID'],row['alt_name'])
