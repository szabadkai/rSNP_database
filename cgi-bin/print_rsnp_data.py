...
with con:
    ...
    for form_data in split_input(field):
        ...
        cur.execute(""" SELECT DISTINCT RS.TFBS_ID FROM TFBS, RS WHERE TFBS.TFBS_ID IN (SELECT DISTINCT TFBS_ID FROM RS WHERE rs_num = '%s') AND TFBS.TFBS_ID = RS.TFBS_ID;""" % form_data)
        rows = cur.fetchall()
